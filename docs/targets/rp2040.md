...
The RP2040 is a dual-core Cortex-M0+ microcontroller from Raspberry Pi with 264 KB SRAM and 2 MB flash (external). iotai-sdk supports the RP2040 with the `thumbv6m-none-eabi` target.
...
cargo build -p iotai-sdk --features cortex-m --target thumbv6m-none-eabi
```
...
```rust
fn read_rp2040_reset_reason() -> iotai_sdk::RebootReason {
