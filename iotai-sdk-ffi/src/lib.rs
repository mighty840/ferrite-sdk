...
use iotai_sdk::fault::{FaultType, FaultRecord, ExceptionFrame, ExtendedRegisters, RamRegion};
use iotai_sdk::reboot_reason::RebootReason;
use iotai_sdk::upload::{UploadManager, UploadStats};
use iotai_sdk::sdk::SdkConfig;
...
impl From<iotai_sdk::SdkError> for IotaiError {
    fn from(e: iotai_sdk::SdkError) -> Self {
        match e {
            iotai_sdk::SdkError::NotInitialized => IotaiError::NotInitialized,
            iotai_sdk::SdkError::AlreadyInitialized => IotaiError::AlreadyInit,
            iotai_sdk::SdkError::BufferFull => IotaiError::BufferFull,
            iotai_sdk::SdkError::KeyTooLong => IotaiError::KeyTooLong,
            iotai_sdk::SdkError::EncodingFailed => IotaiError::Encoding,
...
/// valid for the duration of any `iotai_upload` call.
#[repr(C)]
...
#[no_mangle]
pub unsafe extern "C" fn iotai_sdk_init(
    ...
    if iotai_sdk::sdk::is_initialized() {
...
#[no_mangle]
pub unsafe extern "C" fn iotai_record_reboot_reason(reason: u8) -> IotaiError {
    if !iotai_sdk::sdk::is_initialized() {
        return IotaiError::NotInitialized;
    }
    iotai_sdk::reboot_reason::record_reboot_reason(RebootReason::from(reason));
    IotaiError::Ok
}
...
#[no_mangle]
pub unsafe extern "C" fn iotai_last_reboot_reason(out_reason: *mut u8) -> IotaiError {
    if out_reason.is_null() {
        return IotaiError::NullPtr;
    }
    match iotai_sdk::reboot_reason::last_reboot_reason() {
        Some(r) => {
            *out_reason = r as u8;
        }
        None => {
...
#[no_mangle]
pub unsafe extern "C" fn iotai_metric_increment(
    key: *const c_char,
    delta: u32,
) -> IotaiError {
    if key.is_null() {
    ...
    let ticks = iotai_sdk::metrics::ticks();
    match iotai_sdk::sdk::try_with_sdk(|state| state.metrics.increment(key_str, delta, ticks)) {
        Ok(Ok(())) => IotaiError::Ok,
...
/// `key` is a NUL-terminated C string (max 32 characters).
#[no_mangle]
pub unsafe extern "C" fn iotai_metric_gauge(
    key: *const c_char,
    value: f32,
) -> IotaiError {
    if key.is_null() {
    ...
    let key_str = cstr_to_str(key);
    let ticks = iotai_sdk::metrics::ticks();
    match iotai_sdk::sdk::try_with_sdk(|state| state.metrics.gauge(key_str, value, ticks)) {
        Ok(Ok(())) => IotaiError::Ok,
        Ok(Err(e)) => IotaiError::from(e),
        Err(e) => IotaiError::from(e),
    }
...
/// `key` is a NUL-terminated C string (max 32 characters).
#[no_mangle]
pub unsafe extern "C" fn iotai_metric_observe(
    key: *const c_char,
    value: f32,
) -> IotaiError {
    if key.is_null() {
    ...
    let key_str = cstr_to_str(key);
    let ticks = iotai_sdk::metrics::ticks();
    match iotai_sdk::sdk::try_with_sdk(|state| state.metrics.observe(key_str, value, ticks)) {
        Ok(Ok(())) => IotaiError::Ok,
        Ok(Err(e)) => IotaiError::from(e),
        Err(e) => IotaiError::from(e),
    }
...
/// C-visible fault record returned by `iotai_last_fault`.
#[repr(C)]
#[derive(Debug, Clone, Copy)]
...
#[no_mangle]
pub unsafe extern "C" fn iotai_last_fault(out: *mut IotaiFaultRecord) -> IotaiError {
    if out.is_null() {
        return IotaiError::NullPtr;
    }

    match iotai_sdk::fault::last_fault() {
        Some(record) => {
...
/// Statistics returned by `iotai_upload`.
#[repr(C)]
#[derive(Debug, Clone, Copy)]
...
#[no_mangle]
pub unsafe extern "C" fn iotai_upload(
    ...
    match UploadManager::upload(&mut adapter) {
        ...
        Err(iotai_sdk::upload::UploadError::NotInitialized) => IotaiError::NotInitialized,
        Err(iotai_sdk::upload::UploadError::TransportUnavailable) => IotaiError::Transport,
        Err(iotai_sdk::upload::UploadError::TransportError(_)) => IotaiError::Transport,
        Err(iotai_sdk::upload::UploadError::EncodingError) => IotaiError::Encoding,
    }
}
