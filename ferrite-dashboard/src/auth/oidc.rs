...
impl OidcClient {
    ...
    pub async fn start_login(&mut self) {
        ...
        if let Some(storage) = web_sys::window()
            ...
            .flatten()
        {
            let _ = storage.set_item("ferrite_pkce_verifier", &verifier);
        }

        let auth_url = format!(
            "{}/authorize?response_type=code&client_id={}&redirect_uri={}&scope=openid%20profile&code_challenge={}&code_challenge_method=S256&state=ferrite",
    ...
    pub async fn handle_callback(&self, code: &str) -> Result<TokenSet, String> {
        ...
        {
            storage
                .get_item("ferrite_pkce_verifier")
                .ok()
                .flatten()
