//! Embassy helpers for periodic and on-demand IoTAI data upload.
//!
...
pub async fn upload_loop<T: AsyncChunkTransport>(
    ...
    loop {
        ...
        match UploadManager::upload_async(&mut transport).await {
            ...
            Err(_e) => {
                #[cfg(feature = "defmt")]
                defmt::warn!("iotai upload failed: {}", defmt::Debug2Format(&_e));
            }
        }
    }
}
...
pub async fn upload_loop_with_trigger<T: AsyncChunkTransport>(
    ...
    loop {
        ...
        match &_reason {
            Either::First(()) => {
                defmt::trace!("iotai upload: periodic interval elapsed");
            }
            Either::Second(()) => {
                defmt::trace!("iotai upload: triggered by UPLOAD_NOW");
            }
        }
        let _ = &_reason;
